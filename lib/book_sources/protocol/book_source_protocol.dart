...
class BookSourceChapterContent {
  ...
  /// 漫画/图片章节的图片地址列表（按阅读顺序）。文本为空时以图片为准。
  final List<String>? imageUrls;

  /// 听书章节的音频地址列表（通常为单集一个地址）。
  ...
  factory BookSourceChapterContent.fromJson(Map<String, dynamic> json) {
    ...
    return BookSourceChapterContent(
      ...
      audioUrls: _optionalStringList(json['audioUrls']),
    );
  }
  ...
  Map<String, dynamic> toJson() => {
    ...
    if (audioUrls != null) 'audioUrls': audioUrls,
  };
}

List<String>? _optionalStringList(Object? value) {
  if (value is! List) return null;
  final items = value.whereType<String>().map((e) => e.trim()).toList();
