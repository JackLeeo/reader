...
class BookSourceRegistry {
  ...
  /// Serializes mutations with a completer gate instead of a raw future
  /// chain: gate completion is plain state, so a mutation queued after a
  /// previous finished one starts immediately in the caller's zone without
  /// relying on cross-zone microtask scheduling (which widget tests break).
  static Completer<void> _mutationGate = Completer<void>()..complete();

  Future<T> _mutate<T>(Future<T> Function() action) {
    final completer = Completer<T>();
    final previousGate = _mutationGate;
    final gate = Completer<void>();
    _mutationGate = gate;

    Future<void> run([Object? _]) async {
      try {
    ...
    if (previousGate.isCompleted) {
      ...
      previousGate.future.whenComplete(run);
    }
