...
String readableBookSourceChapterText(
  ...
  // 媒体章节（漫画/听书）由阅读器的媒体视图渲染，这里只产出
  // 占位文本，避免把 URL 列表当正文排版出来。
  final imageCount = content.imageUrls?.length ?? 0;
  final audioCount = content.audioUrls?.length ?? 0;
  if (imageCount > 0 || audioCount > 0) {
    return imageCount > 0 ? '（图像章节 · 共 $imageCount 页）' : '（音频章节）';
  }
