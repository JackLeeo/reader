...
class _SourceSearchPageState extends State<SourceSearchPage> {
  ...
  List<String> _failedSourceNames = const [];
  ...
  /// 书名+作者归一化后作为同一本书的判定键。
  static String _dedupeKey(SourcedBook item) {
    final title = item.book.title.toLowerCase().replaceAll(' ', '');
    final author = (item.book.author).toLowerCase().replaceAll(' ', '');
    return '$title|$author';
  }

  static List<SourcedBook> _dedupeAcrossSources(Iterable<SourcedBook> items) {
    final seen = <String>{};
    return items
        .where((item) => seen.add(_dedupeKey(item)))
        .toList(growable: false);
  }
  ...
  Future<void> _loadMore() async {
    ...
    // 与首轮一致的跨书去重键，保证分页不会引入重复条目。
    final seen = _results.map(_dedupeKey).toSet();
    ...
    for (final batch in batches) {
      ...
      for (final item in batch.items) {
        if (seen.add(_dedupeKey(item))) appended.add(item);
      }
    }
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            Text(
              _totalSourceCount > 0
                  ? '已搜索 $_completedSourceCount/$_totalSourceCount 个书源'
                  : context.l10n.bookSourcesSearch,
              style: TextStyle(color: scheme.onSurfaceVariant),
            ),
            if (_failedSourceNames.isNotEmpty) ...[
              const SizedBox(height: 8),
              Text(
                '${_failedSourceNames.length} 个源失败：'
                '${_failedSourceNames.take(6).join('、')}'
                '${_failedSourceNames.length > 6 ? ' 等' : ''}',
      ...
      slivers: [
        ...
        if (_failedSourceCount > 0)
          SliverPadding(
            ...
            sliver: SliverToBoxAdapter(
              child: Text(
                // 失败源具名：只给数量无法定位是哪个源坏了。
                '${context.l10n.bookSourcesFailedCount(_failedSourceCount)}'
                '${_failedSourceNames.isEmpty ? '' : '：${_failedSourceNames.take(6).join('、')}${_failedSourceNames.length > 6 ? ' 等' : ''}'}',
                style: TextStyle(color: scheme.error, fontSize: 12),
              ),
            ),
          ),
