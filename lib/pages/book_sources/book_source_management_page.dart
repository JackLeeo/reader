...
class _BookSourceManagementPageState extends State<BookSourceManagementPage> {
  ...
  String _searchQuery = '';
  _BookSourceStatusFilter _statusFilter = _BookSourceStatusFilter.all;
  ...
  /// 搜索 + 状态筛选栏。300 个源装进列表后，没有检索能力就无法
  /// 快速定位目标；筛选让批量禁用后的"漏网之鱼"一眼可见。
  Widget _buildSourceFilterBar() {
    final scheme = Theme.of(context).colorScheme;
    final visibleCount = _filterSources(_sources).length;
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        TextField(
          key: const Key('bookSourcesSearchField'),
        ...
        Row(
          children: [
            SegmentedButton<_BookSourceStatusFilter>(
              segments: const [
                ButtonSegment(
                  value: _BookSourceStatusFilter.all,
                  label: Text('全部'),
                ),
                ButtonSegment(
                  value: _BookSourceStatusFilter.enabled,
                  label: Text('已启用'),
                ),
                ButtonSegment(
                  value: _BookSourceStatusFilter.disabled,
                  label: Text('已禁用'),
                ),
              ],
  ...
  List<Widget> _buildSourceGroups(bool additionalProtocolsEnabled) {
    final filtered = _filterSources(_sources);
    final orsp = filtered
    ...
    return [
      ...
      if (orsp.isEmpty && additional.isEmpty)
        Center(
          child: Padding(
...
enum _BookSourceStatusFilter { all, enabled, disabled }
