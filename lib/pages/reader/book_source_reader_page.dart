...
class _BookSourceReaderPageState extends State<BookSourceReaderPage>
  ...
  Widget _buildBody() {
    ...
    if (imageUrls != null && imageUrls.isNotEmpty) {
      return BookSourceImageChapterView(
        ...
        onPreviousChapter: () =>
            _loadChapter(_chapterIndex - 1, saveCurrent: false),
        onNextChapter: () => _loadChapter(_chapterIndex + 1),
        theme: _readerTheme,
        onToggleControls: _toggleControls,
        httpHeaders: _mediaHttpHeaders(),
      );
    }
    final audioUrls = content.audioUrls;
    if (audioUrls != null && audioUrls.isNotEmpty) {
      return BookSourceAudioChapterView(
        ...
        onNextChapter: () => _loadChapter(_chapterIndex + 1),
        theme: _readerTheme,
        onShowCatalog: () => unawaited(_showCatalog()),
      );
    }
    if (_pageMode == BookSourcePageMode.verticalScroll) {
  ...
  /// 媒体章节（漫画图片/听书音频）请求头：书源 header 字段 +
  /// Referer 兜底（防盗链站点通常要求同源 Referer）。
  Map<String, String> _mediaHttpHeaders() {
    final headers = <String, String>{};
    final config = widget.source.sourceConfig;
    final rawHeader = config?['header'];
    Object? decoded = rawHeader;
    if (rawHeader is String && rawHeader.trim().isNotEmpty) {
      try {
        decoded = jsonDecode(rawHeader);
      } on FormatException {
        decoded = null;
      }
    }
    if (decoded is Map) {
      for (final entry in decoded.entries) {
        ...
        if (name.isNotEmpty && entry.value is String) {
