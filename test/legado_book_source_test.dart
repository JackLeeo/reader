...
void main() {
  ...
  test('preflight distinguishes runnable, partial, and blocked sources', () {
    ...
    // 漫画源(2)由媒体阅读视图支持，听书源(1)由音频播放视图支持。
    final image = scanner.scan(
      LegadoBookSource.fromJson(_source(type: 2, name: 'Images')),
    );
    expect(image.level, LegadoCompatibilityLevel.supported);
    expect(image.canRun, isTrue);

    // 视频源超出阅读器能力范围，仍然拦截。
    expect(
      scanner.scan(LegadoBookSource.fromJson(_source(type: 4))).level,
      LegadoCompatibilityLevel.unsupported,
    );
    expect(
      scanner.scan(LegadoBookSource.fromJson(_source(type: 1))).level,
      LegadoCompatibilityLevel.supported,
    );

    // CookieJar 与 JS 引擎接入后，这两类源升级为可运行。
    expect(
      scanner.scan(LegadoBookSource.fromJson(_source(cookies: true))).level,
      LegadoCompatibilityLevel.supported,
    );
    expect(
      scanner
          .scan(LegadoBookSource.fromJson(_source(contentRule: '@js:result')))
          .level,
      LegadoCompatibilityLevel.supported,
    );
  });
  ...
  test(
    ...
    () async {
      ...
      // 自定义构建：更多书源协议默认开启，直接可运行。
      expect(await registry.loadRunnable(), hasLength(1));

      final prefs = await SharedPreferences.getInstance();
      await prefs.setBool(additionalSourceProtocolsPreferenceKey, false);
