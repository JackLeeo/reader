...
void main() {
  ...
  group('LegadoRuleEngine', () {
    ...
    test('accepts explicit and additive CSS prefixes', () async {
      final document = LegadoRuleDocument.parse(
    ...
    test('supports XPath and recursive JSONPath rules', () async {
      final document = LegadoRuleDocument.parse(
        '<div class="shelf"><a href="/1">第一本</a><a href="/2">第二本</a></div>',
        Uri.parse('https://books.test/'),
      );
      final links = await engine.evaluateList(
        document,
        null,
        '//div[@class="shelf"]//a',
      );
      ...
      expect(
        await engine.evaluateString(document, links.first, '//a@href'),
        '/1',
      );

      final json = LegadoRuleDocument.parse(
